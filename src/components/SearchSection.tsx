...
const SearchSection = () => {
  ...
  return (
    <div className="px-4 md:px-6 pb-6 pt-2 mt-24">
      <h1 className="text-2xl md:text-5xl font-bold text-center mb-6">
        {tabValues.map((tab) => (
          <>{tab.value === activeTab && tab.heading}</>
        ))}
      </h1>

      <div className="w-full flex justify-center mt-8 mb-4 overflow-x-auto pb-2 hide-scrollbar">
        <Tabs
          value={activeTab}
          ...
          <TabsList className="bg-transparent flex justify-start overflow-x-auto p-0 space-x-1">
            {tabValues.map(({ value, label, icon: Icon }) => (
              ...
              >
                ...
                <span className="text-[17.5px]">{label}</span>
              </TabsTrigger>
            ))}
          </TabsList>
        </Tabs>
      ...
      <div ref={searchBarRef} className="relative max-w-3xl mx-auto">
        <div
        ...
        >
          <div className="relative flex items-center px-4 py-3">
            ...
            <input
              ...
              className="pl-10 pr-20 py-2 w-full outline-none text-sm placeholder:text-base"
              onClick={() => setIsExpanded(true)}
            />
            {!isExpanded && (
              <button className="search-button absolute right-1 top-1/2 -translate-y-1/2 rounded-full">
                Search
              </button>
            )}
          </div>
            ...
            <div className="p-4 border-t border-gray-200">
              <div className="mb-6">
                <div className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-lg cursor-pointer">
                  <span className="size-12 bg-tripadvisor-gray ring-2 ring-tripadvisor-darkGray/40 flex items-center justify-center rounded-lg shadow-lg"><NavigationIcon className="size-5" /></span>
